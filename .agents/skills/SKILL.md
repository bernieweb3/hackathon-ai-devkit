...
### Phase 4 — Project Planning & Documentation
...
8. Run `hackathon-task-planner` — decompose MVP into tasks with roles and critical path
9. Run `hackathon-doc-writer` — generate PRD and any required ADRs
10. Run `hackathon-code-implementer` — iterate through each task; generate scaffolds and done criteria
11. Run `hackathon-test-generator` — generate demo-path tests and manual verification checklist
12. Run `hackathon-pitchdeck` — construct full pitch deck with judging alignment
13. Run `hackathon-demo-video` — produce demo video script and shot list
14. Run `hackathon-judge-simulator` — stress-test pitch; surface adversarial questions
15. Run `hackathon-submission-prep` — compile and validate all submission artifacts
